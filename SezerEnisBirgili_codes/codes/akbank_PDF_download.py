...
with open(LINK_LIST_PATH, "r") as f:
    ...
    url_list  = [x[0] for x in list_content]
    date_list = [x[1] for x in list_content]
...
for url, date, page in zip(url_list, date_list, page_list):

    response = requests.get(url, headers=headers)

    pdf_path = "{}akbank_{}_{}_{}.pdf".format(PDF_LIST_PATH, date, page, count)
